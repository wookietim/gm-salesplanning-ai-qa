# SSPLAN-730 Test Plan — Weekly Sales Chart has week 36 twice

Mode: **test-plan-only** (Bob planning only; Susan execution deferred)  
Ticket: **SSPLAN-730**  
Last Jira sync: 2026-08-06T18:12:14.904+0000

## 1) Jira snapshot
- Summary: Weekly Sales Chart has week 36 twice
- Issue type: Bug
- Status: In Progress
- Updated: 2026-08-06T18:12:14.904+0000
- Subtasks: none

## 2) Acceptance criteria (normalized)
- **AC-1** Weekly Sales Chart has week 36 twice renders according to design and required states.
- **AC-2** Weekly Sales Chart has week 36 twice handles empty/error data safely without UI breakage.
- **AC-3** Weekly Sales Chart has week 36 twice uses correct filters/parameters for selected hierarchy level.
- **AC-4** Weekly Sales Chart has week 36 twice remains accessible and localized with existing dashboard patterns.
...
|---|---|---|---|---|---|---|
| HP-01 | happy path | REAL FE | Primary Weekly Sales Chart has week 36 twice render path | Open target route with valid fixture/user context. | Expected primary content appears with correct title/value labels and no console/runtime error. | AC-1, AC-2 |
| HP-02 | happy path | REAL FE | Interaction path for Weekly Sales Chart has week 36 twice | Execute expected user interaction (navigate/select/toggle/expand). | State transition completes within 1 click/gesture and target view/data updates correctly. | AC-2, AC-3 |
