...
- **AC-1** Weekly views should not show current week renders according to design and required states.
- **AC-2** Weekly views should not show current week handles empty/error data safely without UI breakage.
- **AC-3** Weekly views should not show current week uses correct filters/parameters for selected hierarchy level.
- **AC-4** Weekly views should not show current week remains accessible and localized with existing dashboard patterns.
...
|---|---|---|---|---|---|---|
| HP-01 | happy path | REAL FE | Primary Weekly views should not show current week render path | Open target route with valid fixture/user context. | Expected primary content appears with correct title/value labels and no console/runtime error. | AC-1, AC-2 |
| HP-02 | happy path | REAL FE | Interaction path for Weekly views should not show current week | Execute expected user interaction (navigate/select/toggle/expand). | State transition completes within 1 click/gesture and target view/data updates correctly. | AC-2, AC-3 |
