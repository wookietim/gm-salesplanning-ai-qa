...
- Name: salesplanning-frontend.spec.js >> Acceptance criteria from Jira stories >> SSPLAN-691 — Segmented Control on all levels >> SegmentControl with all 4 options appears in SalesByWeek on the country dashboard
- Location: salesplanning-frontend.spec.js:1286:9
...
Locator: getByLabel('Sales by week').getByRole('button', { name: 'Qty index', exact: true })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByLabel('Sales by week').getByRole('button', { name: 'Qty index', exact: true })
