...
- Name: salesplanning-frontend.spec.js >> API data → UI binding >> API error responses produce UI error messages (not blank/stale data) >> API error on HFB page does not affect breadcrumbs
- Location: salesplanning-frontend.spec.js:2505:9
...
Locator: getByLabel('Breadcrumb').getByText('HFB 01')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByLabel('Breadcrumb').getByText('HFB 01')
