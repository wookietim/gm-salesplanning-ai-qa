...
- Name: salesplanning-frontend.spec.js >> API data → UI binding >> Weekly metrics API data → Sales by week chart >> switching to "Qty" segment does not produce an error when metrics data is available
- Location: salesplanning-frontend.spec.js:2386:9
...
Call log:
  - waiting for getByLabel('Sales by week').getByRole('button', { name: 'Qty', exact: true })
